 ...
 *
 * Adding a custom model used to mean hand-editing the speckit.customModels
 * array in settings.json: the VS Code settings UI renders an array-of-
 * objects setting as an "Edit in settings.json" link and nothing else, so
 * every field - including which of them are required and what a valid
 * baseUrl looks like - had to be typed blind, with no feedback until an
 * actual document transform failed. This panel is that same setting with a
 * real form: add/remove entries, discover the endpoint's real model ids,
 * test reachability before saving, and order every provider - each custom
 * model individually, by name, alongside Copilot/Claude/Kiro/Generic -
 * instead of ordering one opaque "custom" token that stands for all of
 * them at once.
 *
 * All validation/normalization lives in services/customModelsEditor.ts and
 * services/providerPriority.ts so it can be unit-tested; this file is the
...
import { discoverModels, probeChatCompletion } from '../services/modelDiscoveryService';
import { diagnoseEndpointError, normalizeBaseUrl, normalizeEntriesForSave } from '../services/customModelsEditor';
import { describePriority, filterPriorityTokens } from '../services/providerPriority';

export class CustomModelsPanel {
  public static readonly viewType = 'speckit.customModelsEditor';
  private static current: CustomModelsPanel | undefined;

  private readonly disposables: vscode.Disposable[] = [];
  /** True while the form holds edits the user hasn't saved yet, so an
   * external settings change (e.g. the discovery command writing back a
   * model list) doesn't reload the panel out from under them. */
  private dirty = false;

  /**
   * Opens the editor, revealing the existing panel if one is already open -
   * two panels editing the same settings would let the second one silently
  ...
  public static show(
    ...
    const panel = vscode.window.createWebviewPanel(
      ...
      {
        enableScripts: true,
        // The form holds unsaved edits; without this VS Code tears the
        // webview down whenever the tab is hidden and they're lost.
        retainContextWhenHidden: true
      }
    );

    CustomModelsPanel.current = new CustomModelsPanel(panel, configManager, notificationService);
  }
  ...
  private constructor(
    ...
    // Keeps the form in sync when settings change elsewhere (settings.json,
    // the "Discover Models" command) - but never while the user has
    // unsaved edits open.
    this.disposables.push(
      vscode.workspace.onDidChangeConfiguration((event) => {
        if (!this.dirty && event.affectsConfiguration('speckit')) {
          this.postState();
        }
      })
    );
  ...
  private postState(): void {
    ...
    void this.panel.webview.postMessage({
      type: 'state',
      entries: config.customModels,
      // describePriority expands a bare "custom" token into one row per
      // model, so the list the user sees is always per-model even when the
      // stored setting is still the shipped default.
      priorityRows: describePriority(config.providerPriority, config.customModels)
    });
  }
  ...
  private async save(rawEntries: unknown, rawPriority: unknown): Promise<void> {
    ...
    try {
      await this.configManager.updateConfig('customModels', entries);
      // Not updateConfig(): the try order lives in globalState, not in a
      // setting - see PROVIDER_ORDER_STORAGE_KEY in services/config.ts.
      await this.configManager.updateProviderPriority(priority);
    } catch (error: any) {
      void this.panel.webview.postMessage({
        type: 'saveFailed',
        message: `Could not write settings: ${error.message}`
      });
      return;
    }
    ...
    this.notificationService.info(
      `Saved ${entries.length} custom model entr${entries.length === 1 ? 'y' : 'ies'} ` +
        `(${entries.filter((e) => e.enabled).length} enabled). Try order: ${priority.join(' > ')}`
    );
  ...
  private async discover(rowId: string, baseUrl: string, apiKey: string): Promise<void> {
    ...
    this.notificationService.info(`Discovering models at ${url}...`);
    try {
      const models = await discoverModels(url, apiKey || undefined);
      this.notificationService.info(`Found ${models.length} model(s) at ${url}: ${models.join(', ')}`);
      void this.panel.webview.postMessage({ type: 'discovered', rowId, models });
    } catch (error: any) {
      this.notificationService.error(new Error(`Model discovery failed for ${url}: ${error.message}`));
      void this.panel.webview.postMessage({
        type: 'discovered',
        rowId,
        error: withHint(url, error.message, error.message)
      });
    }
  }

  /**
   * Tests an entry by sending a real (tiny) chat completion, not by
   * listing models: a /models listing succeeding says nothing about
   * whether generation is permitted - a live Google AI Studio endpoint
   * listed its models while every transform came back 403
   * PERMISSION_DENIED, so this button reported "Endpoint reachable" for a
   * provider that could not process a single document.
   *
   * The listing is still fetched, but only to populate the model picker
   * and to note a modelName the endpoint doesn't advertise; the verdict
  ...
  private async test(
    ...
    // Nothing to send a completion for yet - report the listing so the
    // user can pick a model, and be explicit that this is not a pass.
    if (!model) {
    ...
    this.notificationService.info(`Testing ${url} with model "${model}"...`);
    try {
      const sample = await probeChatCompletion(url, key, model);
      const unlisted = models && !models.includes(model)
        ? ` (the endpoint's /models listing doesn't advertise "${model}", but it answered anyway)`
        : '';
      const preview = sample.trim().slice(0, 60);
      this.notificationService.info(`Test succeeded for ${url} (model: ${model})`);
      void this.panel.webview.postMessage({
        type: 'tested',
        rowId,
        ok: true,
      ...
      this.notificationService.error(new Error(`Test failed for ${url} (model: ${model}): ${error.message}`));
      void this.panel.webview.postMessage({
        type: 'tested',
        rowId,
        ok: false,
        message: withHint(url, `Request failed - ${error.message}`, error.message),
        models
      });
    }
  }
  ...
  /**
   * The whole UI is inlined rather than shipped as separate media files:
   * the build is a bare `tsc -p ./` with no asset-copying step, so a
   * media/ directory would never reach out/ and the packaged extension
   * would load a blank panel.
   */
...
<body>
  ...
  <p class="intro">
    ...
    <code>speckit.customModels</code> in your user settings; the try order below is stored by the
    extension itself, so it is edited here and nowhere else.
  </p>
...
(function () {
  ...
  /** @type {Array<Object>} working copy of speckit.customModels */
  let entries = [];
